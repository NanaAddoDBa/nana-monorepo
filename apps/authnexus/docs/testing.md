...
| Backend build | `dotnet build AuthNexus.sln --configuration Release` | All 19 solution projects compile with warnings treated as errors. |
...
builds the solution and runs all three test projects. The downstream mirror request waits for all
three jobs.
...
ApplicationProfile tests run locally without that exception.
